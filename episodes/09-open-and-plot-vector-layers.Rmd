...
- How can I read and visualize point, line and polygon vector data in R?
...
The `st_read()` function gives us a message with information about the file that was read in. Additionally, we can use other functions to examine the metadata of the file in more detail, as we will see in the next section.
...
The `st_crs()` function gives us the coordinate reference system (CRS) used by the shapefile, which in this case is `WGS84` and has the unique reference code `EPSG: 4326`. 
...
`lines_Delft` is in the correct projection. And so is `point_Delft`. As the output of `st_crs()` can be long, you can use `$Name` and `$epsg` after the `crs()` call to extract the projection name and EPSG code respectively.
