...
In the [Plot Raster Data]() episode, we learned how to layer a raster file on top of a hillshade for a nice looking basemap. In that episode, all of our data were in the same CRS. What happens when things don’t line up?
...
We’ll be looking at another model (the canopy/building height model) in [a later episode]() and will see how to calculate the CHM from the DSM and DTM. Here, we will create a map of the Digital Terrain Model (`DTM_TUD`) of TU Delft and its surrounding draped (or layered) on top of the hillshade (`DTM_hill_TUD`). 
...
DTM_hill_TUD <- rast("data/tud-dtm-5m-hill-ETRS89.tif")
```
...
```
`DTM_TUD` is in the Amersfoort / RD New projection, whereas `DTM_hill_TUD` is in ETRS89-extended / LAEA Europe.

:::

:::

Because the two rasters are in different CRSs, they don’t line up when plotted in R. We need to reproject (or change the projection of) `DTM_hill_TUD` into the Amersfoort / RD New CRS.
