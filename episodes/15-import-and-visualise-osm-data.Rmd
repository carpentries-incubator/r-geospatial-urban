...
To map the building ages in Brielle with `Leaflet`, the underlying data remains the same, but we will use the leaflet package to make the map interactive (in other words, our `data` is the R object `buildings` and the visualisation `variable` corresponds to `start_date`).

To do this, you will have to:
- Transform the CRS of `buildings` into WGS84
- Plot a zoomable basemap
- Add the tiles of our choice
- Add polygons (the buildings)
- Use the `fillColor` attribute of these polygons to represent the `build_date` variable.

We can find guidance and examples on how to do that in the following links:
...
Tip: You can also check out the [GDCU cheatsheet](https://github.com/ClementineCttn/r-geospatial-urban/blob/main/instructors/cheatsheet/GDCU_cheatsheet.pdf) for relevant leaflet functions.
