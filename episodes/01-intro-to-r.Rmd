...
highlighted snippet of code will be  executed.
...
The way to do it is to hit <kbd>Esc</kbd>.
...
if the person using your code (e.g. future you) 
is working in a different sub-folder. 


We will use `here()` package to tackle this issue! This package converts relative
...
from the `here` package. In the console, please write:
...
download.file('https://bit.ly/geospatial_data', 
              here('data','gapminder_data.csv'), mode = 'wb')
...
However, what's more useful is the that in R we can store values and
