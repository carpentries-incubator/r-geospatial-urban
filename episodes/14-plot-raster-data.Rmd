...
In this part, we will plot our raster object using `ggplot2` with customized colouring schemes. We will continue working with the Digital Surface Model (DSM) raster from the [previous episode](../episodes/13-intro-to-raster-data.Rmd).
...
ggplot() +
 geom_raster(data = DSM_TUD_df , aes(x = x, y = y,
    ...
    coord_quickmap()
```
The axis labels x and y are not necessary, so we can turn them off by passing `element_blank()` to the `axis.title` argumnet in the `theme()` function.
```{r}
ggplot() +
 geom_raster(data = DSM_TUD_df , aes(x = x, y = y,
                                      fill = fct_elevation_cb)) + 
