...
## Pipe
Now, this is not the most common notation when working with the `dplyr` package. 
Usually, we would use an operator called a pipe: `|>`, which allows you to build up 
very complicated commands in a readable way.
...
But now, we also want to filter the rows of our data set via certain conditions
with `filter()` function. Instead of doing it in separate steps, we can do it all together. 
...
**country** and **year**, only for Europe **or** Asia. 
